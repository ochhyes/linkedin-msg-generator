...
```
Sprint:        #8 — trwała baza profili + auto-backup + dark mode (v1.14.0-1.14.3 COMMITTED) + #49 bulk Resume fix + addCount (v1.14.4 NIESKOMMITOWANE) | Sprint #7 UX redesign #46 wisi (tokeny v1.13.0)
Phase:         Tester/Commit (v1.14.4 zaimplementowane, testy 534/0 PASS, czeka commit + manual smoke 1.14.4 + 1.14.3 + dystrybucja)
Active task:   #49 P0 fix bulk Resume po zamknięciu karty + setting "ile dodać do kolejki" (v1.14.4) — NIESKOMMITOWANE, czeka commit
Last commit:   3542666 — dark/normal mode + baza + hasło dostępu + loading anim (v1.14.3) [+ 0484c65 v1.14.0-1.14.2]
Updated:       2026-05-12 (#49 v1.14.4 zaimplementowane — bulk Resume fix + addCount; czeka commit + manual smoke + dystrybucja)
```
