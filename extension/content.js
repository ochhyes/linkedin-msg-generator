...
(() => {
  ...
  // (v2.0-cleanup) bulkConnectClick + findLiBySlug + waitForShadow usunięte —
  // martwe od #49 v1.14.5 (worker łączy z poziomu profilu, connectFromProfile;
  // klik z listy wyników nie jest wołany przez żaden komponent). Mniej
  // selektorów do utrzymania = mniejsza powierzchnia awarii przy rolloutach.
  ...
  function findConnectEl(root) {
    ...
    for (const c of root.querySelectorAll("a, button")) {
      ...
      if (!t || /W toku|Pending|Anuluj|Withdraw|Cofnij/i.test(t)) continue;
      // Per-LINIA (sr-only spany doklejają tekst do innerText) — lekcja #64.
      if (t.split("\n").some((line) =>
        /^(Połącz|Connect|Nawiąż kontakt)$/i.test(line.trim())
      )) {
        cands.push(c);
